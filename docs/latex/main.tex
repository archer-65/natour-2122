...
L’applicativo è stato progettato basandosi sul concetto di \textbf{Clean architecture}. \\
Tale “clean architecture” è stata realizzata basandosi  sui principi enunciati, nell’omonimo libro,  
da Robert C. Martin, in seguito alla lettura dello stesso. \\
L’idea chiave è quella di usare il principio di inversione di dipendenza per porre dei boundaries tra componenti di alto livello e componenti di basso livello. 
Ciò crea un’architettura \textit{plug-in}, 
che conferisce al sistema un’elevata flessibilità e manutebilità. \\
Un’architettura \textit{pulita} inizia da un codice \textit{pulito}. Per far sì che le classi siano pulite, è 
necessario che lo siano anche le componenti, e dalle componenti pulite deriva un sistema pulito. \\
Il concetto di \textit{clean code} sfruttato nell’implementazione del software segue i 
principi \textbf{SOLID}. Tali principi sono stati applicati in modo da ottimizzare l’architettura 
e ciascuna delle sue componenti, che risultano dunque essere caratterizzate dal:
\begin{itemize}
	\item \textbf{Single responsability principle} – una classe dovrebbe avere uno ed un solo motivo per 
		cambiare;
	\item \textbf{Open-closed principle} – una classe dovrebbe essere aperta all’estensione ma chiusa 
		alle modifiche;
	\item \textbf{Liskov’s substitution principle} – gli oggetti di un programma dovrebbero essere rimpiazzabili 
		  con istanze dei propri sottotipi senza alterare la correttezza del programma stesso;
	\item \textbf{Interface segregation principle} – più interfacce client-spefici sono meglio di un’unica 
		interfaccia general-purpose;
...
\begin{itemize}
	...
	\item Data layer – contenitore delle implementazioni delle sorgenti di dati e dei repository, 
		che coordinano i dati provenienti da esse.
\end{itemize}

È bene sottolineare, però, che nell'effettiva implementazione dell'applicativo non è
stato pedissequamente seguito lo schema appena proposto. È stato adottato, invece, un approccio 
per funzionalità, risultato nell'applicazione del concetto di \textbf{Screaming architecture.}
Essa permette, ad un solo sguardo, di comprendere esattamente la struttura del cuore del codice implementato: 
la suddivisione nei diversi package "urla" al lettore l'approccio utizzato, che risulta dunque chiaro e compresibile.

\newpage
\subsubsection{Pattern architetturali}
La Clean Architecture non preclude, però, la possibilità di adattare all'architettura dell'applicativo un proprio 
design pattern architetturale. Quest'ultima potrebbe infatti essere definita come \textit{ibrida}: ciò è dovuto alla 
