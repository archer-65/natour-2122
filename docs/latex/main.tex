...
\newpage
\section{Codice xUnit per testing di 3 metodi}
\subsection{Metodo \textit{formValidator} - black-box}
...
Nello specifico è stato adottato un approccio \textbf{WECT}, ritenuto sufficiente in quanto non essendovi
correlazioni tra i parametri forniti. \\
...
\subsection{Metodo \textit{getChatMyMembersUseCase} - white-box}

Il metodo \textit{getChatMyMembersUseCase} - che in realtà è implementato attraverso l'override dell'operatore \textit{invoke()} -
ha due parametri: gli id di due utenti registrati.

È stata adottata una strategia white-box, nello specifico con branch coverage,
anche a causa della natura dell'implementazione: altamente dipendente dalle API del linguaggio di programmazione utilizzato.
...
\subsection{Metodo \textit{retrofitSafeCall} - white-box} 

Il metodo \textit{retrofitSafeCall} è il fulcro delle richieste HTTP effettutate dall'applicativo, è stato ritenuto
uno dei metodi più interessanti e particolari da testare data l'enorme riduzione di codice \textit{boilerplate} ottenuta.
