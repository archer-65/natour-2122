...
\begin{itemize}
	...
	\item un client mobile attraverso cui gli
		  ...
		  Gli amministratori, invece, possono anche gestire i contenuti inseriti in piattaforma.\\
\end{itemize}
...
\begin{table}[H]
	...
	\begin{tabular}{ |p{5cm}|p{10.3cm}| }
		...
		\textbf{Nome}              & \textbf{Descrizione}                                                                                 \\
		\hline
		Visualizzazione dettaglio segnalazione & Il sistema deve consentire a un utente autenticato con i privilegi di amministratore 
		di visualizzare i dettagli di una singola segnalazione.                                                                                              \\
		\hline
	\end{tabular}
	\caption{RF.23}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{ |p{5cm}|p{10.3cm}| }
		...
		\textbf{Nome}              & \textbf{Descrizione}                                                                                 \\
		\hline
		Eliminazione segnalazione & Il sistema deve consentire a un utente autenticato con i privilegi di amministratore 
		di eliminare una segnalazione.                                                                                              \\
		\hline
	\end{tabular}
	\caption{RF.24}
\end{table}
...
\begin{itemize}
	\item \textbf{Esposizione} - Posizionare ed organizzare contenuti correlati usando componenti come \textit{card} e \textit{liste};
...
\begin{itemize}
	\item \textbf{Cognito} - Amazon Cognito permette di aggiungere strumenti di registrazione degli utenti, accesso
	      ...
	      Tale risorsa è stata sfruttata relativamente all'autenticazione utente, tenendo particolarmente in considerazione
...
Senza l'utilizzo di Spring, ci sarebbe stata la possibilità che il codice causasse \textit{tight coupling}, che non è
...
La dependency injection è un pattern di programmazione che permette agli sviluppatori di costruire architetture disaccoppiate. In questo caso, Spring comprende le \textit{annotazioni}
...
\textbf{MVVM} sta per Model-View-ViewModel: essa evita l'accoppiamento stretto
tra ciascuna componente. Più nello specifico, in questo tipo di architettura, i figli non hanno un
riferimento diretto al padre, ma hanno solo riferimenti tramite \textit{observables}.\\
...
della moltitudine di observables citata in precedenza. \\
...
Il metodo \textit{retrofitSafeCall} è il fulcro delle richieste HTTP effettuate dall'applicativo. Esso è stato ritenuto
...
Da questa prima analisi è emerso un largo utilizzo della ricerca che, oltre agli itinerari, ha coinvolto anche 
...
Questo è dovuto al fatto che gli strumenti utilizzati non consentano una statistica \textit{totale} in tempo reale.
